package com.animsh.notehut.customlayouts;
...
public class CustomCheckBox extends View implements Checkable {
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (mSmallTick) {
            mTickPoints[0].x = Math.round(widthUnity * 7);
            mTickPoints[0].y = Math.round(heightUnity * 16);

            mTickPoints[1].x = Math.round(widthUnity * 12);
            mTickPoints[1].y = Math.round(heightUnity * 21);

            mTickPoints[2].x = Math.round(widthUnity * 23);
            mTickPoints[2].y = Math.round(heightUnity * 11);
    ...
    private void drawCenter(Canvas canvas) {
        ...
        float radius = (mCenterPoint.x - 4) * mScaleVal;
